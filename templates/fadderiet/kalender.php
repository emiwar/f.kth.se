<?php
function get_mottagningen($startDate='2011-08-14', 
                                   $endDate='2011-09-04') 
{
	load_fadderiet_zend();
	$gdataCal = new Zend_Gdata_Calendar();
	$query = $gdataCal->newEventQuery();
	$query->setUser('med2ps8t9impt3en8cf17rc89s@group.calendar.google.com');
...
$kal = array('halvh' => '20', 'dagb' => '40', 'start' => '8','slut' => '23','startdatum' => 8);
...
function build_week($events,$weekwalker){ 
	...
	<tr class="wk-daynames">
			<th scope="col" title="V.1" class="wk-weekhead"><div class="wk-weekname"><span class="wk-weeklink">V.<?php echo $weekwalker; ?></span></div></th>
			<?php for($i = $kal['walker'];$i <= $kal['walker']+6;$i++){ 
				$datum = $i . ' aug';
				if($i > 31){
					$datum = ($i - 31) . ' sep';	
				}
				?><th scope="col" title="<?php echo $datum; ?>"><div class="wk-dayname"><span class="wk-daylink"><?php echo $datum; ?></span></div></th>
			<?php } ?>
		</tr>
		<tr height="1"><?php //Timmarkeringar ?>
		...
		<tr>
			...
			<?php for($i = 0; $i<7; $i++)://Dagsinehåll ?>
			<td class="tg-col">
				<div style="" class="tg-col-eventwrapper" id="tgCol0">
					<div class="tg-gutter">
						<?php
							get_mottagningen_day($events,$kal['walker']);
							$kal['walker']++;
						?>
					</div>
				</div>
...
function build_calendar($mode = 'grafiskt'){
	global $kal;
	global $eventswalker;
	$events = get_mottagningen('2011-08-'.$kal['startdatum']);
	switch($mode){
		case 'grafiskt':
...
<script type="text/javascript">
	jQuery(document).ready( function($) {
		jQuery(".chip").click(function(e){
			 ...
			 jQuery('.bubble').css({'left': (e.pageX - 107), 'top': (e.pageY - bubbleheight - 75), 'visibility' : 'visible'});
			 e.stopPropagation();
   		});
   		$(".chip").hover(
  			function () {
